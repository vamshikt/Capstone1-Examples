		...
		while (index < inputStream.length) {
			if (inputStream[index++] == CYPHER_ON) {
				if (index < inputStream.length && inputStream[index] == CYPHER_OFF)
					...
					while (index++ < inputStream.length && inputStream[index] == CYPHER_ON) {
						// wait 
					}
				}
				
				if (inputStream[++index] == CYPHER_OFF && inputStream[index] == CYPHER_OFF) {
					morseSymbol.add(CypherSymbol.BLANK);
					morseStream.add(new MorseCharacter(morseSymbol));
					morseSymbol = new ArrayList<>();
					index++;
				}
			}
	...
	private String decodeMorseStream(List<MorseCharacter> morseStream) {
		...
		for (MorseCharacter morseCharacter : morseStream) {
			for (MorseCharacter potentialMatch : morseAlphabet) {
				...
				if (potentialMatch.equals(morseCharacter)) {
